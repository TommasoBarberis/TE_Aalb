...
Description: parse cd-hit-est results. It recovers FASTA sequences for each cluster from the file used to call cd-hit-est. 
Then it separates singleton clusters in 'singleton.fa', low copies number families in 'low_copy_number.fa' and it calls a 
consensus for all other clusters using Refiner (from RepeatModeler2). Consensi sequences can founded in 'consensi.fa' and 
alignements in 'consensi.stk'.
...
Tips:
You can install python dependencies using the .yml in the root directory of the project (https://github.com/TommasoBarberis/TE_Aalb):
conda env create -f TE_Aalb.yml
And then make them available using:
conda activate TE_Aalb

But the conda env contains a lot of other stuffs, so install packages independently will be faster.
"""
...
def refiner(cluster_file, seqs_id, seqs_dict, consensi, stk, max_seq):
    """
...
def call_consensus(cdhit_dict, seqs_dict, out_dir, min_size, ncpu):
    ...
    It uses dictionaries generated by parse_cdhit and parse_fasta functions. 
    out_dir: output directory.
    min_size: threshold to separate low copy number clusters and "normal" clusters
    ...
    with open(out_dir+"/stats.txt", "w") as stats:
        ...
        Number of cluster with a low number of copies (< {min_size}):\t{str(nb_low_copy)}
        Number of cluster with {min_size} or more sequences:\t{str(nb_cluster)}
        """)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description='''
parse cd-hit-est results. It recovers FASTA sequences for each cluster from the file used to call cd-hit-est. 
Then it separates singleton clusters in 'singleton.fa', low copies number families in 'low_copy_number.fa' and it calls a 
consensus for all other clusters using Refiner (from RepeatModeler2). Consensi sequences can founded in 'consensi.fa' and 
alignements in 'consensi.stk'.
        '''
    ...
    cd-hit-est output file (the file having clusters and not the FASTA file having representatives sequences). 
    '''))
    parser.add_argument("--fasta", metavar='', help=('''
    FASTA file used for the clustering with cd-hit-est.
    '''))
    ...
    If you have clustered copies (not consensi) you can run the program with this option set to "1"
    and then you can merge together consensi.fa and singleton.fa.
    '''))
    parser.add_argument("--max_seq", metavar='', default='500', help=('''
    number of sequences that will be sample if the cluster size exceeds this value.
    '''))
